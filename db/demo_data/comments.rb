...
Comment.find_or_create_by(value: "画像付きのコメントです。画像をクリックすると元画像が表示されます。",topic_id: 10,user_id: 1){|c| c.image = open "#{Rails.root}/test/image/200x300.png"}
...
(11..15).each do |i|
  Comment.find_or_create_by(value: "画像付きのコメントです。　#{i}",topic_id: 1,user_id: 1){|c| c.image = open "#{Rails.root}/test/image/200x300.png"}
end
