...
(1..10).each do |i|
  Topic.find_or_create_by(title: "topic #{i}",value: "value #{i}",category_id: 1,user_id: i%2+1)
end

(11..15).each do |i|
  Topic.find_or_create_by(title: "topic #{i}",value: "value #{i}",category_id: 2,user_id: 1)
end

(1..10).each do |i|
  Comment.find_or_create_by(value: "comment #{i}",topic_id: 1,user_id: i%2+1)
end
(11..15).each do |i|
  Comment.find_or_create_by(value: "comment #{i}",topic_id: 11,user_id: 1){|c| c.image = open "test/image/200x300.png"}
end
